...
unsigned int CatheterCmdGrid::getCommandChannel(int cmd_num) const {
    return wxAtoi(GetCellValue(cmd_num, CHANNEL_COL));
}
double CatheterCmdGrid::getCommandCurrentMA(int cmd_num) const {
    return wxAtof(GetCellValue(cmd_num, CURRENT_COL));
}
dir_t CatheterCmdGrid::getCommandDirection(int cmd_num) const {
    return(GetCellValue(cmd_num, DIR_COL).Cmp(dir_choices[DIR_POS]) ? DIR_NEG : DIR_POS);
}
unsigned int CatheterCmdGrid::getCommandDelay(int cmd_num) const {
    return wxAtoi(GetCellValue(cmd_num, DELAY_COL));
}
